...
! !USES:
   ...
   use constants_mod, only: r_vir, cp_vir
...
! Input time independent arrays:
    ...
    real, intent(in):: q(im,jfirst-ng_d:jlast+ng_s,km)
...
!$omp parallel do default(shared) private(k, iord, jord)
    ...
    call geopk(ptop, pe, delpc, pkc, wz, hs(1,jfirst), ptc, im, jm, km,   &
         jfirst, jlast, 0, akap, nx, 0, .false., q)
...
!      call timing_on(' geop_d')
    call geop_d(ptop, pe, delp, pkc, wz, hs(1,jfirst), pt, im, jm,  &
         km,   jfirst, jlast, ng_d, cp, akap, nx, it==n_split, .true., q)
...
!
 ...
 subroutine geop_d(ptop, pe, delp, pk, wz, hs, pt, im, jm, km,  &
                   jfirst, jlast, nd, cp, akap, nx, last_call, dp_check, q)
...
! !INPUT PARAMETERS:
   ...
   real, intent(in)::  q(im,jfirst-nd:jlast+nd,km) 
! !INPUT/OUTPUT PARAMETERS:
...
!              wz(i,j,k) = wz(i,j,k+1) + pt(i,j,k)*(pk(i,j,k+1)-pk(i,j,k))
              kappanew = (1.+r_vir*q(i,j,k))/(1.+cp_vir*q(i,j,k))
              wz(i,j,k) = wz(i,j,k+1) + pt(i,j,k)*&
                          (pk(i,j,k+1)**kappanew-pk(i,j,k)**kappanew)
           enddo
        enddo
2000    continue
        call fv_array_sync()
 ...
 subroutine geopk(ptop, pe, delp, pk, wz, hs, pt, im, jm, km,  &
      jfirst, jlast, nd, akap, nx, id, dp_check, q)
   ...
   real  q(im,jfirst-nd:jlast+nd,km)  ! only altered if dp_check
! !INPUT/OUTPUT PARAMETERS:
