...
use       constants_mod, only: cp_air, hlv, stefan, rdgas, rvgas, grav, cp_vapor
...
! ============================================================================
  ...
  if(use_mixing_ratio) then !default: true
...
!     thv_surf= t_surf0  ! surface virtual (potential) T -- just for testing tun off the q_surf
  ...
  where (avail)
     ...
     drag_t = 5e-3*w_atm ! cd_t * w_atm
     drag_q = 5e-3*w_atm !cd_q * w_atm
     drag_m = 5e-3*w_atm !cd_m * w_atm

     ! density
     !rho = p_atm / (rdgas * tv_atm)  
     rho = p_surf / (rdgas * thv_surf)

     ! sensible heat flux
     rho_drag = (cp_air*(1-q_surf0)+cp_vapor*q_surf0) * drag_t * rho
     flux_t = rho_drag * (t_surf0 - t_atm) !th_atm)  ! flux of sensible heat (W/m**2)
     ...
     where (p_surf .le. e_sat) 
        flux_q = (e_sat - p_surf) / grav / dt / 10.
     elsewhere
        flux_q = rho_drag * (q_surf0 - q_atm)
     endwhere
     !do ii = 1, size(flux_q)
        !if (r_surf(i) .le. 0.) r_surf(i) = 10.
     !   if (flux_q(ii) .le. 0.)   flux_q(ii) = 0.
     !end do
