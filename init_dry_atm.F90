...
! real(r8):: drym = 98290.        ! Revised dry air mass correct for USGS
  ...
  real(r8) :: psini, tslat
  real(r8), dimension(nlev) :: tg_ini, qg_ini, pfull_ini, lnpfull_ini
  real(r8), dimension(nlev+1) :: phalf_ini, lnphalf_ini
...
!df 140109------------------------------------------------------------
!call warm_start(tsini, pdini, 400, psini, t_initial, lnp_initial, q_initial)
!call get_eta_level(nlev, psini, pfull_ini, phalf_ini)
!lnpfull_ini = log(pfull_ini)
!call interp(size(lnp_initial), lnp_initial, t_initial, size(tg_ini), &
!            lnpfull_ini, tg_ini)
...
!end do
      ...
      do j=beglat,endlat
         tslat = tsini-1./(pi/2.)*abs(rlat(1,j))
         call warm_start(tslat, pdini, 400, psini, t_initial, lnp_initial, q_initial)
         call get_eta_level(nlev, psini, pfull_ini, phalf_ini)
         lnpfull_ini = log(pfull_ini)
         call interp(size(lnp_initial), lnp_initial, t_initial, size(tg_ini), &
            lnpfull_ini, tg_ini)
