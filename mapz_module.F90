...
! Compute cp*T + KE
       ...
       do j=js2g0,jn2g0
          ...
          do i=1,im-1
             kappanew = (1.+r_vir*q(i,j,k,1))/(1.+cp_vir*q(i,j,k,1))
             ua(i,j,k) = 0.25 * ( (u2(i,j)+u2(i,j+1))*acosu(j) +   &
                  v2(i,j) + v2(i+1,j)  ) +         &
                  pt(i,j,k)*(pkz(i,j,k)**kappanew)
!                        pt(i,j,k)*pkz(i,j,k)/(1.+r_vir*q(i,j,k,1))  &
!                       * (1.+cp_vir*q(i,j,k,1))
          enddo
! i=im
          kappanew = (1.+r_vir*q(im,j,k,1))/(1.+cp_vir*q(im,j,k,1))
          ua(im,j,k) = 0.25 * ( (u2(im,j)+u2(im,j+1))*acosu(j) +   &
               v2(im,j) + v2(1,j)  ) +            &
               pt(im,j,k)*(pkz(im,j,k)**kappanew)
!                       * (1.+cp_vir*q(im,j,k,1))
!                       * (1.+cp_vir*q(im,j,k,1))
       enddo
...
! SJL: 01/26/04
           ...
           enddo
           kappanew = (1.+r_vir*q(1,1,k,1))/(1.+cp_vir*q(1,1,k,1))
           te_sp = 0.5*te_sp/float(im) +  pt(1,1,k)*pkz(1,1,k)**kappanew
...
! SJL: 01/26/04
           ...
           enddo
           kappanew = (1.+r_vir*q(1,jm,k,1))/(1.+cp_vir*q(1,jm,k,1))
           te_np = 0.5*te_np/float(im) + pt(1,jm,k)*pkz(1,jm,k)**kappanew
...
! Compute va; geopotential increments
       do j=jfirst,jlast
          do i=1,im
             kappanew = (1.+r_vir*q(i,j,k,1))/(1.+cp_vir*q(i,j,k,1))
             va(i,j,k) = pt(i,j,k)*(pk(i,j,k+1)**kappanew-pk(i,j,k)**kappanew)
          enddo
       enddo
...
! use pkz for downward flux
    if(id_dnflux > 0) used = send_data ( id_dnflux,  pkz, Time_next )
    if(id_upflux > 0) used = send_data ( id_upflux, mflux, Time_next )
!      deallocate (mflux)
#endif
