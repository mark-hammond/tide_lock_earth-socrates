...
!parameters for vertical differencing
real :: surf_res        = 0.05 !1
...
!            ak(k) = pt * ( 1. - bk(k) )
        ...
        case (40)
!-----------------------------------------------------------------
! Pure sigma-coordinate with ptop = 0
!-----------------------------------------------------------------
bk(1) = 0.
s2 = 1.-surf_res
scale_heights = log(psg0)
   do k = 1, km
      zeta = 1. - (k - 0.)/km
      z = surf_res *zeta + s2 * (zeta **exponen)
      bk(k+1) = exp(-z*scale_heights)
   end do
ak(2:km+1) = 0.
ks = 0.
ak(1) = 1.
